...
describe('Flow with default mode - callbacks', () => {
  ...
  it('should call ok with an error when stage is not found by index', async () => {
    const syncStageAlpha = {
      handle: jest.fn().mockReturnValue('alpha-result'),
    }
    ...
    await sut.ok(2, spy)
    expect(spy).toHaveBeenCalledTimes(1)
    expect(spy).toHaveBeenCalledWith(new Error('No stage found for stageId 2'))
  })

  it('should call ok with an error when stage is not found by name', async () => {
    const syncStageAlpha = {
      handle: jest.fn().mockReturnValue('alpha-result'),
    }
    ...
    expect(spy).toHaveBeenCalledWith(
      new Error('No stage found for stageId unknown-stage-name'),
    )
  })
})
